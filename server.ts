...
${JSON.stringify(portfolioProjects || [])}`;
    ...
    return res.json({
      ...fallbackResponse,
      quotaWarning: 'Gemini AI quota reached or rate limit exceeded. Using high-precision fallback analysis.'
    });
  }
});
...
app.post('/api/generate-proposal', async (req, res) => {
  ...
  const fallbackProposal = {
    proposalDraft: {
      ...
      body: `I have built similar web applications focused on performance, clean component architecture, and clear client communication.${customInstructions ? ' Note: ' + customInstructions : ''}`,
    ...
    quotaWarning: 'Gemini AI quota reached or rate limit exceeded. Using heuristic fallback proposal.'
  };
