...
export const ProposalsListView: React.FC<ProposalsListViewProps> = ({
  ...
  return (
    <div className="space-y-6 font-sans text-slate-900 dark:text-slate-100 pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
        <div>
          ...
          <p className="text-xs text-slate-600 dark:text-slate-400 font-sans mt-1">
            Every job worked on in <span className="font-semibold text-slate-900 dark:text-slate-200">salmanziachattha107's workspace</span>. Each row opens the full set of generated drafts for that job.
          </p>
        </div>

        <button
          onClick={() => onSelectJob?.('new')}
          className="px-4 py-2 bg-slate-900 dark:bg-slate-100 hover:bg-slate-800 dark:hover:bg-slate-200 text-white dark:text-slate-900 rounded-sm text-xs font-mono font-bold cursor-pointer transition-all self-start sm:self-auto"
        >
          New proposal
        </button>
      </div>
      ...
      {/* Filter Toolbar */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-3 rounded-sm shadow-2xs font-mono text-xs">
        
        {/* Outcome Filter Tabs */}
        <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800/80 p-1 rounded-sm border border-slate-200 dark:border-slate-700 overflow-x-auto">
          <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase px-2">OUTCOME</span>
          {(['ALL', 'VIEWED', 'RESPONSE', 'REJECTED', 'WON'] as const).map(tab => (
            <button
              ...
              className={`px-3 py-1 rounded-sm transition-colors cursor-pointer text-[11px] ${
                outcomeFilter === tab ? 'bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900 font-bold' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
              }`}
            >
              {tab}
            </button>
        ...
        <div className="flex items-center gap-2 flex-1 max-w-md">
          <input
            ...
            className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-sm px-3 py-1.5 text-xs text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-slate-400 dark:focus:border-slate-500"
          />
        </div>
      ...
      <div className="space-y-3 font-mono">
        {filteredProposals.map(p => (
          <div
            ...
            className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 rounded-sm p-4 sm:p-5 transition-all shadow-2xs cursor-pointer flex flex-col md:flex-row md:items-start justify-between gap-4 group"
          >
            <div className="flex items-start gap-4">
              ...
              <div className="space-y-1.5">
                <div className="font-sans font-bold text-slate-900 dark:text-slate-100 text-base group-hover:underline">
                  {p.title}
                </div>
                <div className="text-xs text-slate-500 dark:text-slate-400 font-sans">
                  {p.posted} — <span className="text-slate-600 dark:text-slate-300">{p.summary}</span>
                </div>
                <div className="text-[11px] text-slate-400 dark:text-slate-500 flex flex-wrap items-center gap-3 pt-1">
                  <span>{p.date}</span>
                  <span>·</span>
                  <span className="text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 px-1.5 py-0.2 rounded-sm">{p.status}</span>
                  <span>·</span>
                  <span>{p.model}</span>
                  <span>·</span>
                  <span className="text-slate-700 dark:text-slate-300 font-semibold">{p.profile}</span>
                  <span>·</span>
            ...
            <div className="shrink-0 text-right self-end md:self-start">
              <span className="text-xs font-bold text-slate-700 dark:text-slate-300 group-hover:text-slate-900 dark:group-hover:text-white underline">
                Open draft →
              </span>
            </div>
          </div>
